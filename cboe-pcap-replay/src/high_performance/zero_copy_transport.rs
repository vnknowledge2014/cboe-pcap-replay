...
impl ZeroCopyTransport {
    ...
    fn configure_zero_copy_socket(socket: &Socket, buffer_size: usize) -> Result<()> {
        ...
        // Disable Nagle's algorithm for low latency (TCP only, skip for UDP)
        // Note: set_nodelay() only works for TCP sockets, not UDP
        ...
        #[cfg(target_os = "linux")]
        {
            // Enable zero-copy sending on Linux (if supported)
            use std::os::unix::io::AsRawFd;
            let fd = socket.as_raw_fd();
            
            // Try to set SO_ZEROCOPY (ignore errors if not supported)
            ...
            unsafe {
                ...
                // SO_ZEROCOPY might not be available on all Linux kernels
                if result < 0 {
                    debug!("SO_ZEROCOPY not supported on this system, continuing with standard sockets");
                }
            }
        }
        ...
        #[cfg(target_os = "macos")]
        {
            ...
            let large_buffer = (buffer_size * 2) as libc::c_int;
            unsafe {
                let result = libc::setsockopt(
                    fd,
                    libc::SOL_SOCKET,
                    ...
                    std::mem::size_of_val(&large_buffer) as libc::socklen_t,
                );
                if result < 0 {
                    debug!("Failed to set large send buffer on macOS, using default");
                }
            }
        }
